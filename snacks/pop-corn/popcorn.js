...
recheck.innerHTML = `<a href="popcorn.html"><button> Recheck my order </button></a>`;
...
function setCard() {
    ...
    let plate = ` pack of popcorn <hr class="notify-line">`;
...
function setPackSelection() {
    if(input1.value.length > 0) {
        const packSelectionMessage = document.getElementById("packSelection")
        const packselectCard = document.createElement("div");
        packselectCard.classList.add("packSelectionCard");

        const introMessage = document.getElementById("packIntro");

        fixedImage.style.display = "none";
        fixedName.style.display = "none";

        let introduce = `Hi there, we have two kinds of flavoured packs in the popcorn menu`;
        
        introMessage.innerHTML = `<p class="flavour-title"> ${introduce} </p>`;

        packselectCard.appendChild(introMessage)
        packSelectionMessage.appendChild(packselectCard);

        // packSelectionMessage.style.display = "none"
    }
}

button.addEventListener("click", setConditionForCard);
input1.addEventListener("keyup", setPackSelection);